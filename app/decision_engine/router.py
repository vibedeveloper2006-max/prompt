...
def _calculate_edge_cost(
    ...
    if constraints:
        ...
        if "prefer_fastest" in constraints:
            congestion_penalty = int(congestion_penalty * 0.1) # Fast exit still cares slightly about safety
            trend_penalty = 0
    ...
    while pq:
        ...
        for neighbor, distance in neighbors.items():
            if neighbor not in visited:
                ...
                trend = predictions.get(neighbor, {}).get("trend", "STABLE") if predictions else "STABLE"
                edge_cost = _calculate_edge_cost(distance, score, constraints, priority, neighbor, trend)
