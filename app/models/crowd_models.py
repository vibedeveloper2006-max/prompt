...
class ZoneCrowdStatus(BaseModel):
    ...
    zone_id: str = Field(..., description="Unique shorthand ID for the zone (e.g. 'A1', 'FC')")
    name: str = Field(..., description="Human-readable name of the zone")
    density: int = Field(
        ..., ge=0, le=100, description="Crowd density as a percentage (0-100%)", example=45
    )
    status: str = Field(
        ..., description="Categorical status: LOW (0-39), MEDIUM (40-69), or HIGH (70+)", example="MEDIUM"
    )


class CrowdStatusResponse(BaseModel):
    """Response containing live density data for all stadium nodes."""
    timestamp: datetime = Field(..., description="Server-side time when the snapshot was generated")
    zones: List[ZoneCrowdStatus] = Field(..., description="List of all zone status objects")
...
class CrowdPredictionResponse(BaseModel):
    ...
    predicted_density: int = Field(..., description="Predicted density percentage in 30 minutes")
    trend: str = Field(..., description="Directional movement: INCREASING | STABLE | DECREASING")
    prediction_window_minutes: int = Field(30, description="Look-ahead duration in minutes")
...
class ServiceWaitTime(BaseModel):
    ...
    trend: str = Field(..., description="Wait time trend: INCREASING | DECREASING | STABLE")
...
class WaitTimeResponse(BaseModel):
    ...
    services: List[ServiceWaitTime] = Field(..., description="List of service wait-time objects")
